...
Treball de Fi de Grau estudia el valor i les precaucions d'aquestes tècniques en biblioteques,
arxius, conservació, consulta o preparació editorial.

Es va auditar Sheet Music Benchmark (SMB) i es va reservar una mostra intacta per a la comparació
principal: 64 pàgines de 64 grups documentals i sis degradacions
sintètiques, ampliació dos i quatre amb severitat neta, moderada i forta. Per a comparar
la severitat entre pàgines, el desenfocament es va normalitzar per la separació
entre línies de pentagrama. Es van comparar la interpolació bicúbica, la xarxa residual EDSR i el
transformador lleuger SwinIR mitjançant PSNR i SSIM sobre luminància, intervals aparellats per grup,
...
Es va executar després un estudi secundari d'ajust fi
d'EDSR. Es van definir rols interns amb 45 grups d'entrenament, 13 de validació i 20 de
prova, sense solapament d'identificadors ni reutilització dels 64 grups principals.
Els grups poden ser moviments d'una mateixa composició: no garanteixen independència
musical i els intervals són descriptius, condicionats a l'agrupació. El model adaptat va superar
l'EDSR oficial en PSNR-Y i SSIM-Y per a les sis condicions, amb intervals aparellats que no van
creuar zero. Això dona suport a l'adaptació sobre documents retinguts d'SMB amb degradacions
coincidents, sense certificar generalització a composicions mai vistes ni correcció musical.

El model adaptat es va aplicar sense reajustar-lo a dotze obres d'un
...
mitjana. EDSR ofereix el compromís més defensable entre fidelitat i cost per a generar
...
entrada autònoma d'OMR: cal conservar l'original i sotmetre cada derivat rellevant a revisió humana.
